...
def main():
    ...
    path = r'data\tif_folder'
    outpath = r'data\processed_tif_folder'
    ...
    for file in os.listdir(path):
        gc.enable()
        gc.collect()
        sentinel_1 = ProductIO.readProduct(os.path.join(path, file))
        print(sentinel_1)
        ...
        if (modestamp == 'IW' and productstamp == 'GRDH') or (modestamp == 'EW' and productstamp == 'GRDH'):
            down_tercorrected = do_terrain_correction(down_filtered, proj, 0)
        ...
        if down is None:
            ...
            print("Writing gee image...")
            ProductIO.writeProduct(down_tercorrected, os.path.join(outpath,file[:-5]), 'GeoTIFF')
            del down_tercorrected
        else:
            print("Error.")
